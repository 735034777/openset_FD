...
def plot_pctran_youdenindex(ax):
    ...
    data = {
        "X": [
            ...
            0.039231077, 0.133974596, 0.233035011, 0.367544468, 0.422649731,  # Set 2
            0.039231077, 0.147197135, 0.233035011, 0.3827866, 0.445299804,  # Set 3
            0.039231077, 0.133974596, 0.257218647, 0.3827866, 0.445299804  # Set 4
        ],
        "Y1": [
            0.9793, 0.9305, 0.8741, 0.8660, 0.8298,  # Set 1
            0.9624, 0.9433, 0.8873, 0.8466, 0.7906,  # Set 2
            0.9759, 0.9768, 0.9206, 0.8785, 0.7834,  # Set 3
            0.9586, 0.9196, 0.8922, 0.8717, 0.7956  # Set 4
        ],
        "Y2": [
            0.9759, 0.9257, 0.8612, 0.8668, 0.8764,  # Set 1
            0.9553, 0.9500, 0.8866, 0.8197, 0.7947,  # Set 2
            0.9743, 0.9688, 0.9193, 0.8755, 0.8100,  # Set 3
            0.9580, 0.9050, 0.8977, 0.8533, 0.7956  # Set 4
        ],
        "Y3": [
            0.9606, 0.9349, 0.8476, 0.8805, 0.8623,  # Set 1
            0.9557, 0.9570, 0.8784, 0.8300, 0.7909,  # Set 2
            0.9651, 0.9528, 0.9101, 0.8766, 0.7961,  # Set 3
            0.9580, 0.9166, 0.8968, 0.8392, 0.7961  # Set 4
        ]
    }
    # Predefined labels, markers, and colors for up to 12 groups
    markers = ['o' for i in range(12)]
    colors = ['blue', 'blue', 'blue', 'forestgreen', 'forestgreen', 'forestgreen','maroon','maroon','maroon','crimson','crimson','crimson',
              ]
    labels = [
    ...
    for idx, (key, group_data) in enumerate(grouped_data.items()):
        ...
        ax.plot(group_data['X'], group_data['Y2'], "s",color=colors[color_idx*3+1], linestyle='-',label=labels[idx*3+1])
        ax.plot(group_data['X'], group_data['Y3'], "^",color=colors[color_idx*3+2], linestyle='-',label=labels[idx*3+2])
...
if __name__=="__main__":
    ...
    data1 = {
        "X": [0.039231077, 0.133974596, 0.244071054, 0.340619527, 0.434314575],
        "Y1": [0.9793, 0.9305, 0.8741, 0.8660, 0.8298],
        "Y2": [0.9759, 0.9257, 0.8612, 0.8668, 0.8764],
        "Y3": [0.9606, 0.9349, 0.8476, 0.8805, 0.8623]
    }
    data2 = {
        "X": [0.039231077, 0.133974596, 0.233035011, 0.367544468, 0.422649731],
        "Y1": [0.9624, 0.943333822, 0.887335756, 0.846575342, 0.790555155],
        "Y2": [0.9553, 0.949954216, 0.886607419, 0.819726027, 0.794655732],
        "Y3": [0.9557, 0.957032452, 0.878446115, 0.83, 0.790915645]
    }
    data3 = {
