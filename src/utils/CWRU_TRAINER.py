            ...
            try:
                for metric_types in metric_types_list:
                    ...
                    for i in range(0, 1):
                        ...
                        testaccuracy, precision, recall, f1, youdens_index, soft_accuracy, soft_metrix = calculate_openmax_accuracy(
                            metric_types, 5 + 10 * i)
                        ...
                        new_row = [trainlabels, testlabels, metric_types_record, 5 + 10 * i, open_fault, openness,
                                   accuracy, testaccuracy, precision,
                                   recall, f1, youdens_index, soft_accuracy, soft_metrix]
                        print(
                            "metric is {},tail size is {}, open_fault is {{{}}},accuracy is {:.5f}".format(metric_types,
                                                                                                           5 + 10 * i,
                                                                                                           open_fault,
                                                                                                           testaccuracy))
                        # 确定新行的索引位置
                    ...
                    if os.path.exists(self.save_lmnn_path):
                        os.remove(self.save_lmnn_path)
            except ValueError:
                continue
