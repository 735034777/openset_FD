        ...
        for metric_types in metric_types_list:
            for j in range(2):
                ...
                for i in range(0,10):
                    ...
                    new_row = [trainlabels,testlabels,metric_types_record,5*i,open_fault,openness,accuracy,testaccuracy,precision,
                               recall,f1,youdens_index,soft_accuracy,soft_metrix]
                    print("metric is {},tail size is {}, open_fault is {{{}}},accuracy is {:.5f}".format(metric_types,5*i,open_fault,testaccuracy))
                    # 确定新行的索引位置
                    # new_index = len(results)
                    # 扩展DataFramel
